...
**核心策略**：渐进式上下文加载，初始审查使用 diff，按需提升上下文深度。

**渐进式加载步骤**：

1. **轻量级**：仅需部分文件的全局结构
   - 使用 `get_file_contents(owner, repo, path, ref)` 获取线上单个文件
   - 适用场景：需要查看完整类定义、接口继承关系

2. **重量级**：需要完整代码仓上下文
   - 下载整个 diff 为 patch，应用到本地代码仓
   - 在本地进行深度上下文理解和跨文件分析

**按需加载触发条件**（主动评估是否需要升级上下文）：
- 无法确定影响范围（如：修改了私有方法但不确定调用方）
- 缺少上下文信息（如：只看到方法签名，没有完整实现）
- 需要理解继承/依赖关系（如：修改接口方法，需查看实现类）
- 拿不准的问题（基于现有信息无法判断）

**报告要求**：明确说明加载了哪个层级的上下文及原因。
